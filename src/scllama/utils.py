...
def query_llama_with_image_path(
    ...
    """
    Send an image-aware prompt to a local llama.cpp
    server using file-based image reference.

    This function constructs a prompt in the format
    `/image path.jpg\nYour prompt` and sends it
    to the `/v1/completions` endpoint, assuming
    that the model supports vision input via
    image path syntax.

    Args:
        image_path (str):
        Path to the image file on disk. Must be
        accessible by the server.
        prompt (str):
            The text prompt to accompany the image.
        server_url (str, optional)
             The local llama.cpp completion endpoint.
             Defaults to
    ...
    Returns:
        Dict:
            ...
            typically containing a "completion" field.
    ...
    try:
        ...
        for choice in response["choices"]:
            if "text" in choice:
                ...
                raise ValueError(
                    "No recognizable text or message content in response choice"
                )
